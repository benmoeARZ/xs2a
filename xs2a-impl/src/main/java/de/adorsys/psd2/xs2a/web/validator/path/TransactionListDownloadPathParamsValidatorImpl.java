...
public class TransactionListDownloadPathParamsValidatorImpl implements TransactionListDownloadPathParamsValidator {
    ...
    private static final String DOWNLOAD_URL_SUFFIX_PATH_PARAMETER_NAME = "download-url-suffix";

    private static final String ERROR_TEXT_WRONG_PATH_PARAMETER = String.format("Path parameter '%s' has to be represented by Base64 representation", DOWNLOAD_URL_SUFFIX_PATH_PARAMETER_NAME);
    ...
    public void validate(Map<String, String> queryParameterMap, MessageError messageError) {
        String downloadUrlSuffix = queryParameterMap.get(DOWNLOAD_URL_SUFFIX_PATH_PARAMETER_NAME);

        if (isNonValid(downloadUrlSuffix)) {
